...
int x_board_eth_init(bd_t *bis)
{
	...
	enetaddr[5]++;
	if (eth_env_set_enetaddr("eth1addr", enetaddr)) {
		printf("Failed to set eth1addr\n");
	}

        debug("** board_eth_init DONE\n");

	return ret;
}

/* conflict with file included from ./arch/arm/include/asm/arch/clock.h:10:0? */
...
static int setup_fec(void)
{
	...
	int ret0, ret1;
        ...
        debug("** ts7180.c: setup_fec BEGIN\n");
        /* See board_eth_init(), above for handling of uninitialized MAC addresses */
...
{
        debug("** board_init\n");
	debug("Original Stack Pointer is: %08lx\n", gd->start_addr_sp);
