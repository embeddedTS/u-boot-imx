...
/*
 * Copyright (C) 2020, 2021 Technologic Systems
 */
...
static int8_t i2c_eeprom_read(uint8_t adr, uint16_t subadr, uint8_t *buf,
  ...
  debug("Read: Adr 0x%X, subadr 0x%X\n", adr, subadr);
  ...
  return dm_i2c_read(dev, subadr, buf, len);
}
...
static int8_t i2c_eeprom_write(uint8_t adr, uint16_t subadr, uint8_t *buf,
...
#ifdef DEBUG
  if (len == 1) {
    printf("Write: Adr 0x%X, subadr 0x%X, len %d, buf[0] %02x\n", adr, subadr, len, buf[0]);
  } else {
    printf("Write: Adr 0x%X, subadr 0x%X, len %d\n", adr, subadr, len);
    printf("       ");
    for (int i=0 ; i < len ; i++) {
      printf("%02x ", buf[i]);
    }
  ...
  return dm_i2c_write(dev, subadr, buf, len);
}
...
static uint8_t silab_read_board(uint8_t *buf, int max_chars) {
  int r;

  r = silab_read(4096, buf, max_chars);
  assert(r == 0);
  buf[max_chars - 1] = 0;
...
/* I think this silab_status_ts7180 came from the v2016 tree:
 *
 * Use? Or merge with silab_status_ts7100, above, that is currently
 * used for the 7180 in the v2020 tree?
 */
static void silab_status_ts7180(void) {
  int i, r;
  uint8_t buf[27], build[80];
  uint16_t *sbuf = (uint16_t *)buf;
  uint8_t wdog[4];
  ...
  /* fractions $((536+422)) 422 2500 1024
   * 5 ...
   *  + 5476/10097 -0.000 ppm
   */
  printf("5V: %dmV,\n",
    (((uint32_t)sbuf[0]) * 5) + ((uint32_t)sbuf[0])*5476/10097);

  /* fractions $((422+422)) 422 2500 1024
   * 4 ...
   *  + 113/128 0.000 ppm
   */
  printf("Charge V: %dmV,\n",
    (((uint32_t)sbuf[1]) * 4) + ((uint32_t)sbuf[1])*113/128);
   ...
   */
  printf("3.3V: %dmV,\n",
    (((uint32_t)sbuf[2]) * 4) + ((uint32_t)sbuf[2])*113/128);
   ...
   */
  printf("VIN: %dmV,\n",
    (((uint32_t)sbuf[3]) * 46) + ((uint32_t)sbuf[3])*593/27392);


  printf("SUPERCAP 1: %dmV,\n", sbuf[7]);
  printf("SUPERCAP 2: %dmV,\n", sbuf[8]);
...
static void silab_status(void) {
  if (silab_board_is("7840"))
    ...
    silab_status_ts7100();
  else if (silab_board_is("7180")) {
    uint8_t buf[8];
    silab_read_board(buf, sizeof(buf));
#if 0
    printf("Detected: '%s' (treating as ts7100 == ts7180)\n", buf);
    silab_status_ts7100();
#else
    silab_status_ts7180();
#endif
    ...
    uint8_t buf[8];
    int rc;
    rc = silab_read_board(buf, sizeof(buf));
    if (rc == 0) {
      printf("Board unknown to silabs.c: '%s' (treating as ts7100)\n", buf);
      silab_status_ts7100();
    } else {
      printf("Unrecognized board (Silabs unreadable, rc = %d)", rc);
    }
  }
}
