 ...
 */
#ifndef __TSMFLOW_CONFIG_H
#define __TSMFLOW_CONFIG_H
...
#define CONFIG_OLED_BUS		3 /* ECSPI4 */
#define CONFIG_OLED_CS		0 /* ECSPI4 */
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"cmdline_append=rw rootwait console=ttymxc0,115200 init=/sbin/init loglevel=3\0" \
	"altbootcmd=echo taking some recovery action\0" \
	"silowaitcharge=if test $chrg_wait = '1';" \
		"then silabs scaps wait pct ${chrg_pct};" \
	"fi;\0" \
	"usbprod=usb start;" \
		"if usb storage;" \
	...
	"emmcboot=echo Booting from the eMMC ...;" \
		"if load mmc 0:1 ${loadaddr} /boot/boot.scr;" \
			...
			"source ${loadaddr};" \
		"fi;" \
		"load mmc 0:1 ${fdtaddr} /boot/imx6ul-${model}.dtb;" \
	...
	"sdboot=echo Booting from the SD card ...;" \
		"if load mmc 1:1 ${loadaddr} /boot/boot.scr;" \
			...
			"source ${loadaddr};" \
		"fi;" \
		"load mmc 1:1 ${fdtaddr} /boot/imx6ul-${model}.dtb;" \
	...
	"nfsboot=echo Booting from NFS ...;" \
		...
		"fi;" \
		"nfs ${fdtaddr} " \
		  "${nfsip}:${nfsroot}/boot/imx6ul-${model}.dtb;"\
	...
	"bootcmd_mfg=echo Booted over USB, running test/prime;" \
		...
		"fi;\0" \
	"update-uboot=dhcp;"\
		"if nfs ${loadaddr} ${nfsip}:${nfsroot}/boot/u-boot-${imx_type}.imx; " \
...
#define CONFIG_G_DNL_PRODUCT_NUM	0x9901
